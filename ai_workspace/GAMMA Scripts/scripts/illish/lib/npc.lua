...
-- COMPANIONS --
  ...
  function NPC.getBlockingCompanions()
    ...
    for i, npc in ipairs(NPC.getCompanions()) do
      ...
      local maxAngle = math.abs(math_deg(math.tan(spacing / 2 / dist)))
      local angle1   = math_deg(db.actor:direction():getH())
      local angle2   = math_deg(dir:getH())
...
-- ACTIONS --
  ...
  function NPC.moveOutOfTheWay(npc, options)
    ...
    for i = 0, options.directions do
      angles[#angles+1] = randomFlip * (options.range / options.directions * i - options.range / 2)
    end

    table.sort(angles, function(a, b)
      return math_abs(a) > math_abs(b)
    end)

    for i, angle in ipairs(angles) do
      local idealDist = math_abs(angle) > 30
