...
On the first real task in this workspace, do not wait for the user to ask for setup.

- Check `.codex-stalker/state/bootstrap_state.json` before relying on generated graphify artifacts.
- If that file is missing, stale, or `ai_workspace/lua-graphify-out/graph.json` is absent, run `scripts/bootstrap_workspace.py` automatically.
- Treat `scripts/bootstrap_workspace.py` as the repo-local first-run bootstrap: it installs `graphifyy`, patches the installed `graphify` package so `*.script` is treated as Lua, builds the global `ai_workspace/lua-graphify-out`, builds focused subfolder maps for Lua-bearing `ai_workspace` roots, refreshes `ai_workspace/map_index.html`, and records state in `.codex-stalker/state/bootstrap_state.json`.
- Use `scripts/bootstrap_workspace.py --force-lua-graph` when the state exists but the Lua graph should be refreshed anyway.
...
- `ai_workspace/vanilla scripts/gamedata/graphify-out/...`
- `ai_workspace/GAMMA Scripts/graphify-out/...`
- `ai_workspace/GAMMA Scripts/scripts/graphify-out/...`
- `ai_workspace/Anomaly-Mod-Configuration-Menu-main/graphify-out/...`
- `ai_workspace/Anomaly-Mod-Configuration-Menu-main/gamedata/graphify-out/...`
- any other focused `ai_workspace/**/graphify-out/...` generated by the workspace bootstrap
...
- If the task is already narrowed to one reference pack or one `ai_workspace` subfolder, prefer that focused `graphify-out` map before the global workspace graph.
...
Preferred helper:
- Use `scripts/graphify_workspace.py all --root ai_workspace` to rebuild the global Lua workspace map, focused subfolder maps, and `ai_workspace/map_index.html`.
- Use `scripts/graphify_workspace.py lua-map --root <path>` for another Lua-first STALKER corpus.
- Use `scripts/graphify_workspace.py submaps --root ai_workspace` to refresh only the focused subfolder maps plus the index.
...
2. Before substantial task work, check `.codex-stalker/state/bootstrap_state.json`. If it is missing, stale, or the Lua graph artifacts are absent, run `scripts/bootstrap_workspace.py` automatically.
...
12. If the workspace or subsystem is large and unfamiliar, or if the user asked for a graph/map/architecture view, check graphify artifacts first. Prefer `ai_workspace/map_index.html` and `ai_workspace/lua-graphify-out` for cross-workspace Lua routing, then switch to the relevant focused `ai_workspace/**/graphify-out` map when the task is already localized to one reference pack or subfolder.
