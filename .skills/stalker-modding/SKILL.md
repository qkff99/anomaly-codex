...
2. Run `scripts/scan_workspace.py` or the platform wrapper for unfamiliar or mixed workspaces.
3. If the task is about an existing mod, ask whether to import a copy into `projects/` or edit the target in place.
4. For copy mode, use `scripts/import_mod.py`; for in-place mode, work directly on the provided path and do not force project creation.
5. If the task needs a new local project, create it with `scripts/init_project.py` and keep work under `projects/<project-name>`.
6. If the task is inside a project, validate `projects/<project-name>/.codex-stalker/project.json` with `scripts/check_project.py`.
7. If needed, validate `.codex-stalker/workspace.json` or `.codex-stalker/workspace.yml` with `scripts/check_overlay.py`.
